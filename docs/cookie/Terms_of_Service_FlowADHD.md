# Terms of Service — FlowADHD
**Effective Date:** April 10, 2026  
**Last Updated:** April 10, 2026

---

## 1. ACCEPTANCE OF TERMS

Welcome to **FlowADHD**! These Terms of Service ("Terms," "ToS") govern your access to and use of the FlowADHD Progressive Web Application (PWA) and related services (collectively, the "Service").

**By creating an account or using FlowADHD, you agree to be bound by these Terms.**

If you do NOT agree to these Terms, you may NOT use the Service.

---

## 2. DEFINITIONS

- **"FlowADHD," "we," "us," "our":** FlowADHD, a productivity PWA operated from Brescia, Italy.
- **"User," "you," "your":** Any individual who accesses or uses the Service.
- **"Account":** A registered user profile with associated data (tasks, notes, preferences).
- **"Content":** Any data, text, images, or information you submit to the Service (Brain Dump, tasks, notes).
- **"Subscription":** The paid plan (€5/month) that grants full access to FlowADHD features.

---

## 3. ELIGIBILITY

### 3.1 Age Requirement
You must be **at least 16 years old** to use FlowADHD.

If you are under 16, you may NOT create an account. If we discover that a user is under 16, we will immediately terminate the account and delete all associated data.

### 3.2 Capacity to Contract
By using FlowADHD, you represent that:
- You have the legal capacity to enter into a binding contract
- You are not prohibited from using the Service under applicable law

---

## 4. ACCOUNT REGISTRATION

### 4.1 Creating an Account
To access FlowADHD, you must:
1. Provide a valid email address
2. Create a secure password
3. Accept these Terms and our Privacy Policy

### 4.2 Account Security
**You are responsible for:**
- Maintaining the confidentiality of your password
- All activities that occur under your account
- Notifying us immediately if you suspect unauthorized access

**We are NOT liable for losses caused by unauthorized use of your account.**

### 4.3 Account Termination by You
You may delete your account at any time from **Settings > Account > Delete Account**.

Upon deletion:
- All your data (tasks, Brain Dump notes, preferences) will be permanently deleted within 30 days
- Your subscription will be canceled (no refund for partial months)
- You will lose access to all Service features

---

## 5. SUBSCRIPTION AND PAYMENT

### 5.1 Pricing
FlowADHD offers a subscription plan:
- **Price:** €5/month (billed monthly)
- **Free Trial:** 14 days (no credit card required during trial)

### 5.2 Payment Processing
Payments are processed through **Stripe**, a third-party payment processor. We do NOT store your credit card details.

By subscribing, you authorize us to charge your payment method:
- Monthly on the anniversary of your subscription start date
- Automatically until you cancel

### 5.3 Cancellation and Refunds
**Cancellation:**
- You can cancel anytime from **Settings > Subscription > Cancel**
- Cancellation takes effect at the end of your current billing period
- You retain access to paid features until the period ends

**Refunds:**
- We offer a **14-day money-back guarantee** for first-time subscribers
- After 14 days, subscriptions are **non-refundable**
- Exceptions may be made at our sole discretion (e.g., service outage)

### 5.4 Price Changes
We may change subscription prices with **30 days' notice** via:
- Email to your registered address
- In-app notification

If you do not accept the new price, you can cancel before it takes effect.

### 5.5 Failed Payments
If your payment fails:
- We will attempt to charge your payment method for **3 consecutive days**
- If payment still fails, your account will be **downgraded to read-only mode**
- After 14 days, your account may be suspended

---

## 6. ACCEPTABLE USE POLICY

### 6.1 Permitted Use
You may use FlowADHD for **personal, non-commercial purposes** to:
- Manage tasks and improve focus
- Use AI features for task decomposition
- Store personal notes and reminders

### 6.2 Prohibited Activities
You may NOT:
- ❌ Use the Service for illegal activities or to harm others
- ❌ Attempt to hack, reverse-engineer, or disrupt the Service
- ❌ Share your account credentials with others
- ❌ Upload malicious code, viruses, or malware
- ❌ Abuse AI features (e.g., spam, jailbreaking, prompt injection)
- ❌ Scrape, crawl, or extract data from the Service using bots
- ❌ Impersonate another person or entity
- ❌ Use the Service to harass, bully, or threaten others

**Violation of these rules may result in immediate account termination without refund.**

---

## 7. INTELLECTUAL PROPERTY

### 7.1 Ownership of the Service
FlowADHD (including code, design, logo, and branding) is owned by us and protected by copyright, trademark, and other intellectual property laws.

**You do NOT acquire any ownership rights by using the Service.**

### 7.2 Your Content
You retain ownership of all Content you submit to FlowADHD (tasks, Brain Dump notes, personal data).

**By using the Service, you grant us a limited license to:**
- Store and process your Content to provide the Service
- Use aggregated, anonymized data to improve the Service
- Display your Content to you across your devices

**We do NOT claim ownership of your Content and will NOT use it for any other purpose.**

### 7.3 AI-Generated Content
Content generated by the AI assistant (task decomposition, suggestions) is provided "as-is" for your personal use. We do not claim ownership, but you acknowledge that AI outputs may be similar to outputs generated for other users.

### 7.4 Feedback and Suggestions
If you provide feedback, bug reports, or feature suggestions, you grant us a **perpetual, royalty-free license** to use, modify, and implement your ideas without compensation.

---

## 8. THIRD-PARTY SERVICES

### 8.1 AI Processing (Anthropic Claude API)
FlowADHD uses **Anthropic's Claude API** to power the Brain Dump and AI assistant features. By using these features, you agree to Anthropic's [Terms of Service](https://www.anthropic.com/legal/terms) and [Privacy Policy](https://www.anthropic.com/legal/privacy).

**We are NOT responsible for Anthropic's service availability or data processing practices.**

### 8.2 Payment Processing (Stripe)
Payments are processed by **Stripe**. By subscribing, you agree to Stripe's [Terms](https://stripe.com/legal) and [Privacy Policy](https://stripe.com/privacy).

### 8.3 External Links
The Service may contain links to third-party websites or services. We do NOT endorse or control these sites and are NOT responsible for their content or practices.

---

## 9. DISCLAIMERS AND LIMITATIONS OF LIABILITY

### 9.1 "AS-IS" Service
FlowADHD is provided **"AS-IS"** and **"AS AVAILABLE"** without warranties of any kind, express or implied, including but not limited to:
- Merchantability
- Fitness for a particular purpose
- Non-infringement
- Uninterrupted or error-free operation

**We do NOT guarantee that:**
- The Service will meet your specific needs
- AI features will always produce accurate results
- The Service will be available 24/7 without downtime

### 9.2 No Medical or Mental Health Advice
**⚠️ IMPORTANT DISCLAIMER:**

FlowADHD is a **productivity tool**, NOT a medical device or mental health treatment.

- The AI assistant is NOT a substitute for professional therapy or medical advice
- We do NOT diagnose, treat, or cure ADHD or any other condition
- Always consult a licensed healthcare provider for medical concerns

**If you are experiencing a mental health crisis, contact emergency services or a crisis hotline immediately.**

### 9.3 Limitation of Liability
**To the maximum extent permitted by law:**

We are NOT liable for any:
- Indirect, incidental, special, consequential, or punitive damages
- Loss of profits, data, or business opportunities
- Damages resulting from Service downtime or data loss

**Our total liability to you for any claim shall NOT exceed the amount you paid us in the 12 months preceding the claim (maximum €60).**

### 9.4 Exceptions (EU Consumer Protection)
The above limitations do NOT apply to:
- Personal injury or death caused by our negligence
- Fraud or fraudulent misrepresentation
- Any liability that cannot be excluded under EU consumer protection laws

---

## 10. INDEMNIFICATION

You agree to **indemnify and hold harmless** FlowADHD, its officers, employees, and partners from any claims, damages, or expenses (including legal fees) arising from:
- Your violation of these Terms
- Your misuse of the Service
- Your violation of third-party rights (e.g., copyright infringement)
- Content you submit to the Service

---

## 11. DATA PRIVACY AND SECURITY

### 11.1 Privacy Policy
Your use of FlowADHD is also governed by our **[Privacy Policy](#)**, which explains how we collect, use, and protect your data.

### 11.2 Data Security
We implement industry-standard security measures, including:
- TLS 1.3 encryption for data in transit
- AES-256 encryption for data at rest
- End-to-end encryption for Brain Dump data

**However, no system is 100% secure.** You acknowledge that data transmission over the Internet carries inherent risks.

### 11.3 Data Backups
We perform regular backups, but **you are responsible for maintaining your own copies of critical data.**

---

## 12. SERVICE AVAILABILITY AND MODIFICATIONS

### 12.1 Downtime and Maintenance
We strive for **99% uptime**, but the Service may be temporarily unavailable due to:
- Scheduled maintenance (announced 24 hours in advance)
- Emergency repairs
- Third-party service failures (e.g., hosting provider outage)

**We are NOT liable for damages caused by temporary unavailability.**

### 12.2 Service Changes
We may modify, update, or discontinue features at any time. We will provide **reasonable notice** for major changes, but we are NOT obligated to maintain specific features indefinitely.

### 12.3 Service Termination
We reserve the right to suspend or terminate the Service entirely with **30 days' notice**. In such cases:
- You will receive a full refund for unused subscription time
- You can export your data before the shutdown date

---

## 13. TERMINATION OF ACCOUNT

### 13.1 Termination by You
You may delete your account at any time (see Section 4.3).

### 13.2 Termination by Us
We may suspend or terminate your account immediately (without refund) if:
- You violate these Terms
- You engage in fraudulent or abusive behavior
- Your account remains inactive for 2+ years with unpaid subscription
- We are required to do so by law

**Before termination, we will provide notice (when legally permissible) and an opportunity to cure the violation.**

### 13.3 Effect of Termination
Upon termination:
- You lose access to the Service immediately
- Your data will be deleted within 30 days
- Sections 7 (Intellectual Property), 9 (Disclaimers), 10 (Indemnification), and 14 (Governing Law) survive termination

---

## 14. GOVERNING LAW AND DISPUTE RESOLUTION

### 14.1 Governing Law
These Terms are governed by the laws of **Italy** and the **European Union**, without regard to conflict of law principles.

### 14.2 Jurisdiction
Any disputes arising from these Terms shall be subject to the **exclusive jurisdiction of the courts of Brescia, Italy**.

### 14.3 Informal Resolution (Encouraged)
Before filing a lawsuit, we encourage you to contact us at **legal@flowadhd.com** to resolve disputes informally.

### 14.4 EU Consumer Rights
If you are a consumer in the EU, you have the right to:
- File a complaint with your local consumer protection authority
- Use the EU Online Dispute Resolution platform: [ec.europa.eu/odr](https://ec.europa.eu/consumers/odr)

---

## 15. MISCELLANEOUS

### 15.1 Entire Agreement
These Terms, together with our Privacy Policy, constitute the entire agreement between you and FlowADHD.

### 15.2 Severability
If any provision of these Terms is found invalid or unenforceable, the remaining provisions will remain in full effect.

### 15.3 Waiver
Our failure to enforce any right or provision does NOT constitute a waiver of that right.

### 15.4 Assignment
You may NOT transfer or assign your account or these Terms. We may assign our rights to a successor or affiliate.

### 15.5 Force Majeure
We are NOT liable for delays or failures caused by events beyond our reasonable control (e.g., natural disasters, war, pandemics, government actions).

---

## 16. CHANGES TO THESE TERMS

We may update these Terms from time to time. Changes will be posted on this page with an updated "Last Updated" date.

**Material Changes:**
- We will notify you via email or in-app notification
- Continued use after changes constitutes acceptance
- If you do not agree, you must stop using the Service and delete your account

---

## 17. CONTACT US

If you have questions about these Terms, contact us:

📧 **Email:** legal@flowadhd.com  
📍 **Mailing Address:**  
FlowADHD  
Via Example, 123  
25100 Brescia (BS)  
Italy

---

## 18. ACKNOWLEDGMENT

**By using FlowADHD, you acknowledge that:**
- You have read and understood these Terms
- You agree to be bound by these Terms
- You are at least 16 years old
- You have the authority to enter into this agreement

If you do not agree, you may not use the Service.

---

*These Terms of Service comply with EU consumer protection laws (Directive 2011/83/EU) and Italian Civil Code (Codice Civile).*

---

**FlowADHD — Created with care for those who see the world differently.** 🌊
